...
const ProfLogin = () => {
  ...
  const FetchUserData = async() => {
    await getDoc(doc(db , 'ProfessorData',ph))
    .then((docSnap) => {
      let usr = docSnap.data().username;
      let clg = docSnap.data().college;
      let clg_code = docSnap.data().clgcode;
      setUname(usr);
      setCollege(clg);
      setClgCode(clg_code);
    })
...
const verifyOTP = async() => {
  ...
  return (
    <div className='w-11/12 md:w-2/5 h-auto mx-auto mt-5 md:mt-16 items-center flex flex-col md:justify-center gap-10 bg-gradient-to-b from-white/10 to-white/80 rounded-md drop-shadow-2xl pb-10 overflow-hidden'>
      <Toaster toastOptions={{ duration: 1000 }} />
      <div id="recaptcha-container"></div>
      <div className='flex flex-col items-center gap-3'>
          <img className='size-16 md:size-24' src={logocircle}  alt="" />
          <p className='text-white md:text-lg font-audiowide'>Professor Login</p>
      </div>
      ...
      <input value={name} onChange={(e) => setName(e.target.value)} className='w-4/5 md:w-1/2 md:h-12 h-10 rounded-md drop-shadow-xl px-3 focus:ring-primary placeholder:text-gray-500' type="text" placeholder='Enter Username' />
        ...
        {showOTP ? (
                ...
                <p className='absolute bottom-3 font-audiowide text-primary text-xs'>New to MSU Result Scraper ? <span> <Link className='hover:underline' to='/ProfessorRegister' >Sign Up</Link></span></p>
      </div>
